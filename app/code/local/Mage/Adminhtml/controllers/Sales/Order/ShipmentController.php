            ...
            if ($shipment) {
                ...
                if ($shipment->getAllTracks()) {
                    foreach ($shipment->getAllTracks() as $carrier) {
                        if ($carrier->getCarrierCode() == 'mondialrelaypickup'
                                || $carrier->getCarrierCode() == 'mondialrelayhome') {
                            $trackingTitle = $this->__('Track Your Order');
                            $url = Mage::getStoreConfig('carriers/mondialrelay/url_tracking') . Mage::getStoreConfig('carriers/mondialrelay/company') . '&nexp=' . $carrier->getNumber();
                            $tracking_order = '<p><a title="' . $trackingTitle . '" href="' . $url . '"><b>' . $trackingTitle . '</b></a></p>';
                        }
                    }
                }
        
                $shipment->sendEmail(true, $tracking_order)
                    ->setEmailSent(true)
                    ->save();
